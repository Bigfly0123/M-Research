# 08｜Self-Reflection Judge 详细设计与实施指导
...
**Self-Reflection Judge** 是 DocResearch-Agent 的 **自反思评测层**。从 relevance、citation_support、faithfulness、context_sufficiency 等维度检查答案可信度并输出 failure_type。
...
LLM-as-a-judge：灵活但必须有 rubric。

### 3.2 Self-reflection

Self-reflection：不是主观评价，而是结构化检查。

### 3.3 Failure attribution

Failure attribution：比单一分数更有用。

### 3.4 Repair action

Repair action：Judge 输出必须能驱动 Repair。
...
当前模块 **Self-Reflection Judge** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Self-Reflection Judge', 'status': 'ok'})
```
...
class JudgeResult(BaseModel):
    ...
    failure_type: Literal['none','retrieval_miss','weak_evidence','citation_error','hallucination','incomplete_answer','context_noise']
    repair_action: str | None
    reason: str
```

阈值建议：relevance >= 0.7，citation_support >= 0.7，faithfulness >= 0.75，context_sufficiency >= 0.6。
