# 11｜Trace + Eval Runner 详细设计与实施指导
...
**Trace + Eval Runner** 是 DocResearch-Agent 的 **可追踪评测闭环层**。记录每个节点输入输出和修复路径，并运行 eval 对比 baseline、hybrid、graph、agentic repair。
...
Trace：是 Agent 系统的黑盒记录。

### 3.2 Eval

Eval：需要 dataset、runner、metrics、report。

### 3.3 Metrics

Metrics：要覆盖 retrieval、generation、repair、cost/latency。

### 3.4 Failure cases

Failure cases：失败案例比平均分更有说服力。
...
当前模块 **Trace + Eval Runner** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Trace + Eval Runner', 'status': 'ok'})
```
...
Trace schema 包含 run_id、question、nodes、final_answer、metrics。

Eval dataset 示例：

```json
{"question":"Answer Judge 如何触发 Repair Router？","expected_keywords":["failure_type","repair_action"],"reference_chunks":["C12"],"query_type":"multi_hop"}
```

指标：retrieval_hit_rate、citation_support_rate、judge_pass_rate、repair_success_rate、avg_latency_ms、avg_context_tokens、failure_distribution。
