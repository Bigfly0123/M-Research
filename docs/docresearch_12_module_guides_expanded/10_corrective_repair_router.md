# 10｜Corrective Repair Router 详细设计与实施指导
...
**Corrective Repair Router** 是 DocResearch-Agent 的 **纠错修复路由层**。根据 failure_type 选择 query rewrite、graph expansion、evidence recomposition、regeneration 等修复动作。
...
Repair 不是 retry：必须按错误类型纠错。

### 3.2 Max repairs

Max repairs：必须设置 max_repairs 防止循环。

### 3.3 Before/after

Before/after：修复前后要记录对比。

### 3.4 Stop condition

Stop condition：有些失败应该停止并保守回答。
...
当前模块 **Corrective Repair Router** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Corrective Repair Router', 'status': 'ok'})
```
...
AgentState 必须有 repair_count、max_repairs、repair_history。超过次数后输出保守答案，不要无限重试。
