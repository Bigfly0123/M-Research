# 05｜Retrieval Evaluator 详细设计与实施指导
...
**Retrieval Evaluator** 是 DocResearch-Agent 的 **检索质量诊断层**。在生成答案前判断检索结果质量，决定继续、重写 query、扩大 top-k、图扩展或 fallback。
...
Evidence quality：相似度高不代表证据能支持答案。

### 3.3 Failure type

Failure type：检索失败要分型，而不是只看 top-k。

### 3.4 Repair dependency

Repair dependency：Evaluator 是 Repair Router 的前置诊断器。
...
当前模块 **Retrieval Evaluator** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Retrieval Evaluator', 'status': 'ok'})
```
...
class RetrievalEvalResult(BaseModel):
    evidence_quality: Literal['strong','weak','irrelevant','conflicting']
    confidence: float
    reason: str
    missing_evidence: list[str] = []
    recommended_action: Literal['continue','rewrite_query','increase_top_k','graph_expand','fallback_bm25']
```

规则先行：候选为空 -> irrelevant；top score 低 -> weak；候选来源冲突 -> conflicting。LLM 再做语义支持判断。
