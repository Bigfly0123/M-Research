...
**Tool Registry** 是 DocResearch-Agent 的 **工具抽象与调用层**。统一管理 dense_search、bm25_search、graph_expand、citation_check、eval_run 等工具。
...
### 3.1 Tool abstraction

Tool abstraction：节点只依赖 schema，不依赖具体实现。

### 3.2 MCP-style design

MCP-style design：工具有 name、description、input_schema、output_schema。

### 3.3 Dependency inversion

Dependency inversion：Graph 节点调用 registry，不直接 import 具体 retriever。

### 3.4 Observability

Observability：每次工具调用都应该记录 trace。
...
当前模块 **Tool Registry** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Tool Registry', 'status': 'ok'})
```
...
初始工具：dense_search、bm25_search、graph_expand、rerank、citation_check、eval_run。第一版不需要实现 MCP server，但要用 MCP-style schema 思维设计。
