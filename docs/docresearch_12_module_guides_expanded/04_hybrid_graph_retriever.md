# 04｜Hybrid + Graph Retriever 详细设计与实施指导
...
**Hybrid + Graph Retriever** 是 DocResearch-Agent 的 **混合图增强检索层**。结合 dense retrieval、BM25、term graph expansion 和 rerank，提高技术文档证据覆盖率。
...
Dense retrieval：擅长语义召回。

### 3.2 BM25

BM25：擅长函数名、参数名、术语精确匹配。

### 3.3 Graph expansion

Graph expansion：用关联术语补充多跳证据。

### 3.4 Fusion/rerank

Fusion/rerank：用于合并多路候选。
...
当前模块 **Hybrid + Graph Retriever** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Hybrid + Graph Retriever', 'status': 'ok'})
```
...
三路召回：Dense Search、BM25 Search、Graph Expansion。

Term graph 第一版用 Python dict：
...
实施顺序：dense -> BM25 -> merge/dedup -> term graph -> graph expansion -> 可选 rerank。

必须做 ablation：dense only vs dense+BM25 vs dense+BM25+graph。
