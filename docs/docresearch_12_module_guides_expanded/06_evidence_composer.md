# 06｜Evidence Composer 详细设计与实施指导
...
**Evidence Composer** 是 DocResearch-Agent 的 **证据组织与上下文压缩层**。把候选 chunk 组织成 evidence pack，完成去重、排序、压缩、citation id 分配和证据角色标注。
...
Context engineering：组织上下文，而不是塞满上下文。

### 3.2 Deduplication

Deduplication：重复证据会浪费预算。

### 3.3 Compression

Compression：压缩必须保留引用来源。

### 3.4 Evidence role

Evidence role：证据角色能帮助生成器写出结构化答案。
...
- LlamaIndex response synthesizers: https://docs.llamaindex.ai/
...
当前模块 **Evidence Composer** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Evidence Composer', 'status': 'ok'})
```
...
class EvidenceItem(BaseModel):
    ...
    role: Literal['definition','procedure','comparison','example','code','limitation']
    support_score: float
```

规则：同 source/section 聚合；重复 chunk 去重；超 budget 时保留 definition/procedure/code；citation_id 稳定，如 D1-C12。
