# 02｜Contextual / Structure-aware Chunker 详细设计与实施指导
...
**Contextual / Structure-aware Chunker** 是 DocResearch-Agent 的 **文档结构化切块层**。把 PDF、Markdown、代码文档解析成保留标题、章节、代码块、表格、页码和上下文摘要的 chunk。
...
Chunking：过大导致噪声，过小导致上下文丢失。

### 3.2 Structure-aware parsing

Structure-aware parsing：标题、章节、代码块、表格、页码都应该进入 metadata。

### 3.3 Contextual chunking

Contextual chunking：为 chunk 补充文档标题、章节路径、局部摘要。

### 3.4 Metadata-aware retrieval

Metadata-aware retrieval：后续可以按 element_type、section、source 过滤。
...
当前模块 **Contextual / Structure-aware Chunker** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Contextual / Structure-aware Chunker', 'status': 'ok'})
```
...
class DocChunk(BaseModel):
    ...
    page: int | None = None
    element_type: Literal['text','heading','code','table'] = 'text'
    text: str
    contextual_text: str
    token_count: int
    metadata: dict = {}
...
Markdown 优先按标题层级切。代码块单独切成 `element_type='code'`。表格单独切成 `element_type='table'`。PDF 第一版按页和段落切，记录 page。

Contextual header 示例：

```text
[Document: DocResearch-Agent Design]
[Section: Hybrid + Graph Retriever / Term Graph]
[Element: code]
Original chunk: ...
```

实施顺序：先支持 Markdown，再支持 PDF，再加 code/table，再生成 contextual_text。
