# 12｜Skill Prompt Registry 详细设计与实施指导
...
**Skill Prompt Registry** 是 DocResearch-Agent 的 **Prompt/Skill 模块化管理层**。把 planner、judge、generator、repair 等 prompt、rubric、schema 拆成可版本化 skill 文件。
...
Prompt registry：把 prompt 从代码中剥离。

### 3.2 Skill-style design

Skill-style design：包括 instructions、schema、rubric、examples。

### 3.3 Versioning

Versioning：版本化 prompt 才能做 A/B test。

### 3.4 Governance

Governance：Prompt 是工程资产，需要治理。



## 4. 技术参考链接

- Anthropic Skills: https://docs.anthropic.com/en/docs/agents-and-tools/skills/overview
...
当前模块 **Skill Prompt Registry** 必须遵守三个原则：输入输出结构化、关键决策写入 trace、不抢其他模块职责。
...
class Module:
    ...
    def run(self, input_data: dict) -> ModuleResult:
        ...
        # 3. build trace
        # 4. return result
        return ModuleResult(data={}, trace={'module': 'Skill Prompt Registry', 'status': 'ok'})
```
...
SkillRegistry 负责读取 instructions、schema、rubric，供各模块调用。这样 prompt 不散落在代码里。
